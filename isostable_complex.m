% This code computes the isostables of the FitzHugh-Nagumo model (case of complex eigenvalues), using forward integration. It runs in about 2 minutes 
...
x_interv = linspace(-0.5,1.5,100);
y_interv = linspace(-0.6,0.4,100);
...
for test = 1:number_loops;
    ...
    % ode integration
    omega = abs(imag(lambda1));
    [t,x] = ode45(f_dyn,[0:2*pi/omega:t_end],init_cond,options);
    
    % observable
    x1 = x(:,1:number_traj_loop);
    x2 = x(:,number_traj_loop+1:2*number_traj_loop);
    f = sqrt((-vec2(2)*(x1-pt_fix(1))+vec2(1)*(x2-pt_fix(2))).^2+(vec1(2)*(x1-pt_fix(1))-vec1(1)*(x2-pt_fix(2))).^2);
    
    % averaging
    average(index_low:index_high) = f(end,:).*(exp(-real(lambda1)*t(end))*ones(1,number_traj_loop));
...
figure(1)
title('Isotables')
contour(x0,y0,-log(abs(data)),0.1:0.1:10,'b')

figure(2)
title('Isochrons')
contour(x0,y0,angle(data),0.1:0.1:10,'b')
